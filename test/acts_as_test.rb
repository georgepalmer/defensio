...
class ActsAsTest < Test::Unit::TestCase
  ...
  def test_audit_comment
    article = build_article :permalink => "http://refactormycode.com/codes/test-#{Time.now.to_i}"
  ...
  def test_annouce_article
    ...
    Article.defensio.
      ...
      once.
      with(:article_author       => article.author,
           :article_author_email => article.author_email,
  ...
  def test_audit_comment_with_defensio_error
    comment = build_comment :article => build_article(:new_record => false)
    Comment.defensio.stubs(:post).returns <<-EOS
        <html>
        <head>
    ...
    comment.save
    assert comment.spam
    assert_equal 0, comment.spaminess
  end
  ...
  private
    def build_article(attributes={})
      Article.new({ :author       => 'marc',
                    ...
                    :content      => '...',
                    :permalink    => 'http://refactormycode.com/codes/test-article',
